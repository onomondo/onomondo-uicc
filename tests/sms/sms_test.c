...
{
	uint8_t sms_tpdu[] = { 0x40, 0x08, 0x81, 0x55, 0x66, 0x77, 0x88, 0x7f, 0xf6, 0x00, 0x11, 0x29, 0x12,
	...
	printf(" tp_scts=%s\n", ss_hexdump(sm_hdr.u.sms_deliver.tp_scts, sizeof(sm_hdr.u.sms_deliver.tp_scts)));
...
void ss_sms_hdr_decode_sms_status_report(void)
{
	uint8_t sms_tpdu[] = { 0x02, 0x23, 0x08, 0x81, 0x55, 0x66, 0x77, 0x88, 0x00, 0x11, 0x29, 0x12,
			       0x00, 0x00, 0x04, 0x00, 0x11, 0x29, 0x12, 0x00, 0x00, 0x04, 0x42 };
	struct ss_sm_hdr sm_hdr;
	int rc;
	...
	printf(" tp_scts=%s\n",
	       ss_hexdump(sm_hdr.u.sms_status_report.tp_scts, sizeof(sm_hdr.u.sms_status_report.tp_scts)));
	printf(" tp_dt=%s\n", ss_hexdump(sm_hdr.u.sms_status_report.tp_dt, sizeof(sm_hdr.u.sms_status_report.tp_dt)));
...
void ss_sms_hdr_encode_test_sms_submit(void)
{
	...
	memset(sm_hdr.u.sms_submit.tp_vp, 0xAA, sizeof(sm_hdr.u.sms_submit.tp_vp));
...
static void sms_tx_state_show(struct ss_context *ctx)
{
	...
	SS_LIST_FOR_EACH(&ctx->proactive.sms_tx_state.sm, sm, struct ss_uicc_sms_tx_sm, list) {
		printf("  SM:%s, last_msg=%s\n", ss_hexdump(sm->msg, sm->msg_len), sm->last_msg ? "true" : "false");
	}
}
...
void ss_uicc_sms_tx_test_single(void)
{
	...
	int rc;
	struct ss_sm_hdr sm_hdr;
	uint8_t tp_ud[] = { 0xc8, 0x22, 0x93, 0xf9, 0x64, 0x5d, 0x9f, 0x52, 0x26, 0x11 };
	...
	rc = ss_uicc_sms_tx(&ctx, &sm_hdr, NULL, 0, tp_ud, sizeof(tp_ud), NULL);
	if (rc < 0)
		assert(false);
...
void ss_uicc_sms_tx_test_multi(void)
{
	...
	printf("test ss_uicc_sms_tx (message that needs to be splitted over multiple SM)\n");
	...
	rc = ss_uicc_sms_tx(&ctx, &sm_hdr, ud_hdr, sizeof(ud_hdr), tp_ud, sizeof(tp_ud), NULL);
	if (rc < 0)
		assert(false);
