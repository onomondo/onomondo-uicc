...
static uint16_t transact_hex_apdu(struct ss_context *ctx, const char *hex,
				  uint8_t *resp, size_t resp_bufsize,
				  size_t *out_resp_len)
{
	uint8_t cmd[300];
	size_t cmd_len = ss_binary_from_hexstr(cmd, sizeof(cmd), hex);
	size_t resp_len =
		ss_application_apdu_transact(ctx, resp, resp_bufsize, cmd, &cmd_len);
	if (out_resp_len)
...
int main(void)
{
	...
	sw = transact_hex_apdu(ctx, "8010000014ffffffffffffffffffffffffffffffffffffffff",
	...
	sw = transact_hex_apdu(ctx, "801400000c810301130082028281830100",
			       resp, sizeof(resp), &resp_len);
