...
static int fix_short_fd(struct ss_list *fcp_decoded_envelope)
{
	...
	if (!fcp_ie_fd) {
		SS_LOGP(SADMIN, LERROR,
			"invalid FCP: no file descriptor (%02X), cannot fix!\n",
	...
	if (!fcp_ie_file_size) {
		SS_LOGP(SADMIN, LERROR,
			"invalid FCP: short file descr -- no size IE (%02X), cannot fix!\n",
	...
	if (fcp_ie_fd->value->len == 4) {
		...
		memcpy(fd_fixed->data, fcp_ie_fd->value->data,
		       fcp_ie_fd->value->len);
		...
		if (rc < 0) {
			...
			SS_LOGP(SADMIN, LERROR,
				"invalid FCP: short file descr -- cannot fix!\n");
			return -EINVAL;
		}
		...
		if (fd_decoded.structure != SS_FCP_LINEAR_FIXED &&
		    fd_decoded.structure != SS_FCP_CYCLIC) {
			ss_buf_free(fd_fixed);
			SS_LOGP(SADMIN, LERROR,
				"invalid FCP: short file descr -- cannot fix!\n");
			return -EINVAL;
		...
		file_size =
		    ss_uint32_from_array(fcp_ie_file_size->value->data,
					 fcp_ie_file_size->value->len);
		...
		SS_LOGP(SADMIN, LERROR,
			"invalid FCP: short file descr -- fixed (%s)\n",
			ss_hexdump(fcp_ie_fd->value->data,
				   fcp_ie_fd->value->len));
		return 0;
	}
...
static int validate_fcp(struct ss_list *fcp_decoded_envelope)
{
	...
	if (!fcp_ie) {
		SS_LOGP(SADMIN, LERROR,
			"invalid FCP: no file descriptor (%02X)\n",
			TS_102_221_IEI_FCP_FILE_DESCR);
		rc = -EINVAL;
	} else {
		...
		if (rc < 0) {
			SS_LOGP(SADMIN, LERROR,
				"invalid FCP: unable to decode file descriptor -- trying to fix (%s)\n",
	...
	if (!fcp_ie) {
		SS_LOGP(SADMIN, LERROR,
			"invalid FCP: no file identifier (%02X)\n",
	...
	fcp_ie =
	    ss_btlv_get_ie_minlen(fcp_decoded_envelope,
				  TS_102_221_IEI_FCP_LIFE_CYCLE_ST, 1);
	if (!fcp_ie) {
		SS_LOGP(SADMIN, LERROR,
			"invalid FCP: no file lifecycle status byte (%02X)\n",
	...
	if (ss_btlv_get_ie(fcp_decoded_envelope, TS_102_221_IEI_FCP_SEC_ATTR_8C)
	    || ss_btlv_get_ie(fcp_decoded_envelope,
			      TS_102_221_IEI_FCP_SEC_ATTR_AB)) {
		SS_LOGP(SADMIN, LERROR,
	...
	fcp_ie =
	    ss_btlv_get_ie(fcp_decoded_envelope, TS_102_221_IEI_FCP_SEC_ATTR_8B);
	if (!fcp_ie) {
		SS_LOGP(SADMIN, LERROR,
			"invalid FCP: no referenced security attributes (%02X)\n",
	...
	case SS_FCP_WORKING_EF:
		...
		default:
			SS_LOGP(SADMIN, LERROR,
				"invalid FCP: unsupported file struture (%u)\n",
	...
	case SS_FCP_DF_OR_ADF:
		...
		if (fcp_ie) {
			SS_LOGP(SADMIN, LERROR,
				"FCP needlessly contains pin status template (%02X)\n",
				TS_102_221_IEI_FCP_PIN_STAT_TMPL);
			rc = -EINVAL;
		}
	...
	default:
		SS_LOGP(SADMIN, LERROR,
			"invalid FCP: unsupported file type (%u)\n",
			file_descr.type);
		rc = -EINVAL;
	}
...
int ss_uicc_admin_cmd_create_file(struct ss_apdu *apdu)
{
	...
	if (!ss_access_check_command(apdu,
				file_descr.type == SS_FCP_DF_OR_ADF ?
				SS_ACCESS_INTENTION_DF_CREATE_DF :
...
int ss_uicc_admin_cmd_activate_file(struct ss_apdu *apdu)
{
	...
	struct ss_file *selected_file = ss_get_file_from_path(&apdu->lchan->fs_path);
	fcp_decoded_lifecycle =
		ss_btlv_get_ie_minlen(selected_file->fcp_decoded, TS_102_221_IEI_FCP_LIFE_CYCLE_ST, 1);
