			     ...
			     uint8_t resp_data_len)
{
	...
	if (rc != 0 && state->retry_counter < MAX_REFRESH_RETRYS) {
		SS_LOGP(SREFRESH, LERROR,
			"unsccessful REFRESH command, retrying...\n");
		...
		SS_LOGP(SREFRESH, LERROR,
			"unsccessful REFRESH command, giving up...\n");
		state->state = SS_REFRESH_READY;
		state->retry_counter = 0;
	} else {
		SS_LOGP(SREFRESH, LDEBUG,
			"successful REFRESH command, done!\n");
		state->state = SS_REFRESH_READY;
		state->retry_counter = 0;
	}
}
...
static int send_refresh(struct ss_context *ctx,
			struct ss_uicc_refresh_state *state)
{
	...
	if (filelist_len < 0) {
		SS_LOGP(SREFRESH, LDEBUG,
			"Sending REFRESH command failed -- file list is invalid!\n");
		return -EINVAL;
	}
	cmd_list = SS_ALLOC(struct ss_list);
	ss_list_init(cmd_list);
	ss_ctlv_new_ie(cmd_list, TS_101_220_IEI_CMD_DETAILS, true,
		       sizeof(cmd_details), cmd_details);
	ss_ctlv_new_ie(cmd_list, TS_101_220_IEI_DEV_ID, true, sizeof(device_id),
		       device_id);
	ss_ctlv_new_ie(cmd_list, TS_101_220_IEI_FILE_LST_OR_CAT_SERV_LST, true,
		       filelist_len, state->filelist);
	...
	if (!cmd) {
		SS_LOGP(SREFRESH, LERROR,
			"Sending REFRESH failed -- cannot encode command!:\n");
...
void ss_uicc_refresh_poll(struct ss_context *ctx)
{
	...
	if (!ss_proactive_term_prof_bit(ctx, 3, 8)) {
		SS_LOGP(SREFRESH, LERROR,
			"cannot refresh files, TERMINAL PROFILE does not support REFRESH command!\n");
		rc = -EINVAL;
		return;
	}
	...
	case SS_REFRESH_READY:
		...
		SS_LOGP(SREFRESH, LDEBUG,
			"following file changes will be refreshed:\n");
		ss_fs_chg_dump(ctx->fs_chg_filelist, 2, SREFRESH, LDEBUG);
		memcpy(state->filelist, ctx->fs_chg_filelist,
		       SS_FS_CHG_BUF_SIZE);
	...
	case SS_REFRESH_PENDING:
		...
		if (rc == -EBUSY) {
			SS_LOGP(SREFRESH, LERROR,
				"cannot send REFRESH, another command is busy, retrying...\n");
			return;
		} else if (rc < 0) {
			...
			SS_LOGP(SREFRESH, LERROR,
				"cannot send REFRESH, data is not accepted, giving up...\n");
			state->state = SS_REFRESH_READY;
			return;
		}
	...
	case SS_REFRESH_TRANSIT:
		...
		SS_LOGP(SREFRESH, LDEBUG,
			"waiting until file changes are transmitted...:\n");
		break;
	}
}
