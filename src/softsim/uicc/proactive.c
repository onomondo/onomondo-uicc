...
static void default_term_response_cb(struct ss_context *ctx,
				     uint8_t *resp_data, uint8_t resp_data_len)
{
	...
	if (!ctlv_data) {
		SS_LOGP(SPROACT, LERROR,
			"Unable to decode response - non valid COMPRESNSION-TLV?\n");
		goto leave;
	}
...
void ss_proactive_poll(struct ss_context *ctx)
{
	...
	if (ctx->proactive.term_resp_cb && ctx->proactive.data_len > 0) {
		...
		if (ctx->proactive.term_resp_poll_ctr >= MAX_POLL_CYCLES) {
			...
			SS_LOGP(SPROACT, LDEBUG,
				"waiting %u poll cycles for TERMINAL RESPONSE\n",
				ctx->proactive.term_resp_poll_ctr);
			ctx->proactive.term_resp_poll_ctr++;
		}
	}
		     ...
		     const uint8_t *data, size_t len)
{
	...
	if (ctx->proactive.data_len) {
		SS_LOGP(SPROACT, LERROR,
			"unable to put data, previous data not fetched yet!\n");
		return -EBUSY;
	}
	...
	ss_btlv_new_ie(proact_cmd, "proactive-command",
		       TS_101_220_IEI_PROACTIVE_CMD, len, data);

	ctx->proactive.data_len =
	    ss_btlv_encode(ctx->proactive.data, sizeof(ctx->proactive.data),
			   proact_cmd);
	if (ctx->proactive.data_len == 0) {
		SS_LOGP(SPROACT, LERROR,
			"unable to put data, cannot encode BER-TLV enevelope!\n");
		rc = -EINVAL;
	}
			...
			enum log_subsys log_subsys)
{
	...
	/* No response at all */
	if (!resp_data) {
		SS_LOGP(log_subsys, LDEBUG,
			"no terminal response received -- command unsuccessful!\n");
		return -EINVAL;
	}
	...
	ctlv_data = ss_ctlv_decode(resp_data, resp_data_len);
	if (!ctlv_data) {
		SS_LOGP(log_subsys, LERROR,
			"Unable to decode response -- command unsuccessful!\n");
		rc = -EINVAL;
		goto leave;
	}
	...
	cmd_result_ie =
	    ss_ctlv_get_ie_minlen(ctlv_data, TS_101_220_IEI_RESULT, 1);
	if (!cmd_result_ie) {
				...
				uint8_t bit_idx)
{
	...
	if (byte_idx == 0) {
		SS_LOGP(SPROACT, LERROR,
			"TERMINAL PROFILE byte indexes start countingt at 1!\n");
		assert(false);
	}
	if (bit_idx == 0) {
		SS_LOGP(SPROACT, LERROR,
			"TERMINAL PROFILE bit indexes start countingt at 1!\n");
		assert(false);
	}
	if (byte_idx > sizeof(ctx->proactive.term_profile)) {
		SS_LOGP(SPROACT, LERROR,
			"Tried to access TERMINAL PROFILE byte %zu, but end ist at byte %zu!\n",
			byte_idx, sizeof(ctx->proactive.term_profile));
		assert(false);
	}
	if (bit_idx > 8) {
		SS_LOGP(SPROACT, LERROR,
			"Tried to access TERMINAL PROFILE byte at bit %u!\n",
			bit_idx);
		assert(false);
	}
